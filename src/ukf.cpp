...
UKF::UKF() {
  ...
  */
  ...
  //set weights here as we don't change the weights
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
  ...
  {
    cout << "Unsupported sensor measurements; Ignore" <<endl;
    return;
  }
...
void UKF::Prediction(double delta_t) {
  ...
  ****************************************************************************/
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //iterate over sigma points
    ...
    if(i == 0)
      P_ = P_ + (weights_(i)+2.9) * x_diff * x_diff.transpose();
    else
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    if ( i == 0)
      S = S + (weights_(i)+2.9) * z_diff * z_diff.transpose();
    else
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  ****************************************************************************/
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    if(i == 0)
      Tc = Tc + (weights_(i)+2.9) * x_diff * z_diff.transpose();
    else
      Tc = Tc + weights_(i) * x_diff * z_diff.transpose();
  }
